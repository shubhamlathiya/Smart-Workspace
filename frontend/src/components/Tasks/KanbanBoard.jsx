...
import {showConfirmAlert} from "../../utils/alerts.jsx";
...
const KanbanBoard = ({projectId}) => {
    ...
    const handleDrop = async (item, newStatus) => {
        ...
        // Restriction logic
        if (oldStatus === 'completed' && newStatus !== 'review') {
            toast.success('Completed tasks can only move to Review.');
            return;
        }

        if (oldStatus === 'review' && newStatus !== 'in-progress' && newStatus !== 'completed') {
            toast.success('Review tasks can only move to In Progress.');
            return;
        }

        if (oldStatus === 'todo' && newStatus === 'completed') {
            const confirmed = await showConfirmAlert({
                title: 'Move task directly to Completed?',
                text: 'This will mark it as fully done. Are you sure?',

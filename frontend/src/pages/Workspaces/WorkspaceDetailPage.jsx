...
import {
    deleteWorkspace, fetchWorkspaceById, removeMember, setCurrentWorkspace, updateWorkspace
...
const WorkspaceDetailPage = () => {
    ...
    const handleDelete = async () => {
        ...
        if (confirmed) {
            try {
                ...
                showErrorAlert('error', 'Failed', `${e} Unable to delete this workspace. Please try again.`);
            }
        }
    };
    ...
    const getUserRoleInWorkspace = (userId, workspace) => {
    ...
    const handleUpdateSettings = async (updatedSettings) => {
        ...
        const role = getUserRoleInWorkspace(user._id, currentWorkspace);

        // Only 'owner' or 'admin' can update settings
        if (role !== 'owner' && role !== 'admin') {
            showErrorAlert('error', 'Access Denied', 'You do not have permission to update workspace settings.');
            return;
        }

        try {
            // Dispatch an action or call API to update settings
            // const result = await dispatch(updateWorkspaceSettings({
            ...
            showSuccessAlert('success', 'Settings Updated', 'Workspace settings updated successfully.');
        } catch (error) {
            console.error(error);
            showErrorAlert('error', 'Update Failed', 'Failed to update workspace settings.');
        }
    };
        ...
        >
            ...
            {activeTab === 'settings' && (
                <div className="space-y-6">
                    ...
                    {(() => {
                        const userRole = getUserRoleInWorkspace(user._id, currentWorkspace); // helper to find role
                                ...
                                <div className="space-y-6">
                                    ...
                                    <div className="glass-card p-6 border border-white/20">
                                        ...
                                        <div className="flex flex-col space-y-3">
                                            <label className="text-white/70 text-sm">Workspace Visibility</label>
                                            <select
                                                value={currentWorkspace.settings.isPublic ? 'public' : 'private'}
                                                onChange={(e) =>
                                                    handleUpdateSettings('isPublic', e.target.value === 'public')
                                                }
                                            ...
                                            <label className="text-white/70 text-sm">Allow Member Invites</label>
                                            <input
                                                type="checkbox"
                                                checked={currentWorkspace.settings.allowMemberInvites}
                                                onChange={(e) =>
                                                    handleUpdateSettings('allowMemberInvites', e.target.checked)
                                                }
                                            />

                                            <label className="text-white/70 text-sm">Default Task Status</label>
                                            <select
                                                value={currentWorkspace.settings.defaultTaskStatus}
                                                onChange={(e) =>
                                                    handleUpdateSettings('defaultTaskStatus', e.target.value)
                                                }
                                                className="p-2 rounded-lg bg-white/10 text-white"
                                            >
